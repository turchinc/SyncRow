...
def create_pr_for_locale(locale: str, missing_data: Dict):
    ...
    if missing_count == 0:
        print(f"✅ No missing strings for {locale.upper()}, skipping PR")
        return
    
    print(f"\n📝 Creating PR for {locale.upper()} ({missing_count} missing strings)...")
...
3. Ensure informal address ("tu"/"du"/"tú") is used as per project guidelines
...
def main():
    ...
    if not results_file.exists():
        print("❌ Results file not found. Run check_localizations.py first.")
        sys.exit(1)
    
    with open(results_file, 'r', encoding='utf-8') as f:
        results = json.load(f)
    
    print("🚀 Creating PRs for locales with missing strings...\n")
