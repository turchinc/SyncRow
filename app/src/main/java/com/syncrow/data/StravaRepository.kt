...
class StravaRepository(
    ...
    suspend fun connect(code: String, user: User) {
        ...
        try {
            val id = clientId.toIntOrNull() ?: throw IllegalStateException("Invalid Strava Client ID: $clientId")
    ...
    suspend fun uploadWorkout(workout: Workout, points: List<MetricPoint>, user: User): Boolean {
        ...
        val name = "SyncRow: ${workout.totalDistanceMeters}m Row".toRequestBody("text/plain".toMediaTypeOrNull())
        ...
        return try {
            val response = stravaApi.uploadActivity(
                "Bearer $token",
                body,
                name,
                description,
