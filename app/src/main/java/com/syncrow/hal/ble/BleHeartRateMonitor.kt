...
class BleHeartRateMonitor(private val rxBleClient: RxBleClient) : IHeartRateMonitor {
  ...
  override fun getHeartRateFlow(targetAddress: String?): Flow<Int> {
    ...
    return rxBleClient
      ...
      .flatMap { scanResult ->
        Log.d(
          "BleHRM",
          "Connecting to HRM: ${scanResult.bleDevice.macAddress} (${scanResult.bleDevice.name})"
        )
        scanResult.bleDevice.establishConnection(false)
      }
      .flatMap { connection -> connection.setupNotification(HEART_RATE_MEASUREMENT_CHAR_UUID) }
      .flatMap { it }
      .map { bytes -> parseHeartRate(bytes) }
      .retryWhen { errors ->
